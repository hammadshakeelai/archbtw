...
import { TextScreen } from "../src/lib/vga.ts";
...
emulator.add_listener("screen-put-char", (args: [number, number, number]) => textScreen.put(args));
emulator.add_listener("screen-set-size", (args: [number, number, number]) => textScreen.resize(args));
...
function dumpScreen(title: string): void {
  console.log(`\n--- ${title} ---`);
  if (textScreen.graphical) console.log("  (the guest is in a graphics mode; there is no text to show)");
  for (const row of textScreen.rows()) console.log(`  | ${row}`);
  console.log("---");
}
...
if (!(await waitFor("the prompt on screen", PROMPT_TIMEOUT_MS, () => textScreen.text().includes(PROMPT)))) {
  await fail(
    textScreen.graphical
      ? "the console switched to a graphics mode, so the snapshot would not show a text prompt"
      : `the shell started but "${PROMPT}" never appeared on screen`,
  );
}
